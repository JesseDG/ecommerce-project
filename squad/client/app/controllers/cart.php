	...
	class cart extends Controller
	{
				...
				foreach ($ordersList as $order_item) {
					...
					$str .= '<tr id="row' . $order_item->ID .'" class="rowToDelete">
							...
							<td class="cart_quantity" >
								<div class="cart_quantity_button" style="vertical-align: middle;display: block; " value="'. $item->item_stock.'" id="maxQuantity">
		...
		public function addToCart($item_id)
		{
			if(isset($_SESSION['user']))
			{
				...
				$item = $this->model('item');
				$query = $item->where(['item_id'=> $item_id, 'status_item'=>Controller::available]);
				
				if(count($query)>0 && $query[0]->item_stock > 0)
				{
					$item = $query[0];
					$orders = $this->model('orders');
					...
					if(count($query) == 0)
					{
						...
						$orders->bought_price = $item->item_price;
		...
		public function deleteFromCart($orderID)
		{
			...
			if(count($query) > 0)
			{
				...
				$sale->total = $sale->total - $price * $qty;
		...
		public function increaseQuantity($ID)
		{
			...
			$item = $this->model('item');
			$queryItem = $item->where(['item_id'=>$item_id]);
			$item = $queryItem[0];
			...
			if($item->item_stock > $orders->quantity)
			{
				$orders->quantity = $orders->quantity + 1;
				$sale->total = $sale->total + $price;
			}
